...
const STOCK_CALC_TIME_SQL = (createdDateField = 'st.CREATED_DATE', codeField = 'st.CODE', weightCodeField = 'st.WEIGHT_CODE', stockDateField = 'st.STOCK_DATE') => `
    CASE 
        WHEN ${weightCodeField} IS NOT NULL THEN DATE_FORMAT(${stockDateField}, '%Y-%m-%d %H:%i:%s')
        WHEN ${codeField} IS NULL 
             OR ${codeField} LIKE 'ADJ-%' 
             OR ${codeField} LIKE 'STOCKOP-%' 
             OR ${codeField} LIKE 'SLO-%'
             ...
             OR ${codeField} LIKE '%-WEB-%'
             OR ${codeField} LIKE '%-SLO-%'
        THEN DATE_FORMAT(CONVERT_TZ(${createdDateField}, '+00:00', '+05:30'), '%Y-%m-%d %H:%i:%s')
        ELSE DATE_FORMAT(${createdDateField}, '%Y-%m-%d %H:%i:%s')
    END
`;
...
router.get('/api/reports-dashboard/clearances/:itemId', async (req, res) => {
    try {
        ...
        const runningStockQuery = `
            SELECT 
                DATE(${STOCK_CALC_TIME_SQL('st.CREATED_DATE', 'st.CODE', 'st.WEIGHT_CODE', 'st.STOCK_DATE')}) as tx_date,
                SUM(CASE 
            ...
            WHERE sti.ITEM_ID = ?
              AND st.IS_ACTIVE = 1
              AND sti.IS_ACTIVE = 1
            GROUP BY DATE(${STOCK_CALC_TIME_SQL('st.CREATED_DATE', 'st.CODE', 'st.WEIGHT_CODE', 'st.STOCK_DATE')})
    ...
    try {
        ...
        const findOpBoundary = async (opId) => {
            ...
            const [latestTx] = await pool.query(
                `SELECT MAX(${STOCK_CALC_TIME_SQL('st.CREATED_DATE', 'st.CODE', 'st.WEIGHT_CODE', 'st.STOCK_DATE')}) as max_date 
                 FROM store_transactions st 
        ...
        for (const storeNo of [1, 2]) {
            const [result] = await pool.query(`
                ...
                WHERE sti.ITEM_ID = ?
                  ...
                  AND st.IS_ACTIVE = 1
                  AND sti.IS_ACTIVE = 1
                  AND ${STOCK_CALC_TIME_SQL('st.CREATED_DATE', 'st.CODE', 'st.WEIGHT_CODE', 'st.STOCK_DATE')} <= ?
        ...
        const dailyQuery = `
            ...
            WHERE sti.ITEM_ID = ?
              ...
              AND st.COMMENTS NOT LIKE '%[S1-%'
              AND st.COMMENTS NOT LIKE '%[WEB-%'
            GROUP BY DATE(${STOCK_CALC_TIME_SQL('st.CREATED_DATE', 'st.CODE', 'st.WEIGHT_CODE', 'st.STOCK_DATE')}), st.TYPE, st.STORE_NO
            ORDER BY tx_date ASC
        `;
...
router.post('/api/graphs/item-data', async (req, res) => {
    try {
        ...
        const runningStockQuery = `
            ...
            GROUP BY DATE(${STOCK_CALC_TIME_SQL('st.CREATED_DATE', 'st.CODE', 'st.WEIGHT_CODE', 'st.STOCK_DATE')}), st.STORE_NO
            ORDER BY tx_date ASC
        `;
...
router.post('/api/graphs/stock-events', async (req, res) => {
    try {
        ...
        const txEventsQuery = `
            SELECT
                ${STOCK_CALC_TIME_SQL('st.CREATED_DATE', 'st.CODE', 'st.WEIGHT_CODE', 'st.STOCK_DATE')} as event_time,
