...
router.post('/api/getDailyDashboardStats', async (req, res) => {
    ...
    try {
        ...
        const todayItemSalesQuery = `
            ...
            AND t.TYPE = 'Selling'
            AND DATE(COALESCE(t.STOCK_DATE, ${SL_TIME_SQL('t.CREATED_DATE', 't.CODE')})) = ?
        ...
        const avgBuyPriceQuery = `
            ...
            WHERE t.IS_ACTIVE = 1 AND sti.IS_ACTIVE = 1
            AND t.TYPE = 'Buying'
            AND DATE(COALESCE(t.STOCK_DATE, ${SL_TIME_SQL('t.CREATED_DATE', 't.CODE')})) BETWEEN DATE_SUB(?, INTERVAL 30 DAY) AND ?
            GROUP BY ITEM_ID
        `;
