    ...
    try {
        ...
        const transactionQuery = `
            ...
            WHERE st.IS_ACTIVE = 1 AND sti.IS_ACTIVE = 1 
              ...
              ${req.body.startDate ? `AND DATE(COALESCE(st.STOCK_DATE, ${SL_TIME_SQL('st.CREATED_DATE', 'st.CODE')})) >= '${req.body.startDate}'` : ''}
              ${req.body.endDate ? `AND DATE(COALESCE(st.STOCK_DATE, ${SL_TIME_SQL('st.CREATED_DATE', 'st.CODE')})) <= '${req.body.endDate}'` : ''}
        `;
