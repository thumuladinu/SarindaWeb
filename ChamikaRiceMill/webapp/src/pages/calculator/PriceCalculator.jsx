...
const defaultConfigState = {
    ...
    loadSizeKg: 10000,      // Total Load Size in kg
...
export default function PriceCalculator() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const [itemsRes, custsRes] = await Promise.all([
                    axios.post('/api/MillgetAllItems').catch(() => axios.get('/api/mill/items')).catch(() => ({ data: {} })),
                    axios.post('/api/MillgetAllCustomers', {}, { withCredentials: true }).catch(() => ({ data: {} }))
                ]);

                const list = itemsRes.data?.result || itemsRes.data?.data || (Array.isArray(itemsRes.data) ? itemsRes.data : []);
                setItemsList(list || []);

                const custs = custsRes.data?.result || custsRes.data?.data || (Array.isArray(custsRes.data) ? custsRes.data : []);
                    ...
                    <Card title={<span className="font-bold text-purple-600 dark:text-purple-400">Step 3: Logistics & Transport Rate</span>} className="shadow-sm">
                        <Row gutter={[12, 12]}>
                            <Col xs={24}>
                                ...
                                >
                                    {customersList.map(c => (
                                        <Option key={c.CUSTOMER_ID || c.id} value={c.CUSTOMER_ID || c.id} label={c.NAME}>
