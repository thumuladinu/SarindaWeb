...
import {
    ...
    Space, Tooltip, message, Modal, Select, Form, Badge
} from 'antd';
import {
...
const LOCAL_STORAGE_KEY = 'chamika_price_calculator_unified_electron_v2';
...
export default function PriceCalculator() {
    ...
    const handleConfirmSavePrices = async () => {
        ...
        try {
            // Update Dexie database offline items
            const targetItem = itemsList.find(x => String(x.ITEM_ID) === String(selectedRiceItemId));
            if (targetItem) {
                await db.items.update(targetItem.ITEM_ID, { SELLING_PRICE: draftRicePrice });
            }

            const hunsalItem = itemsList.find(x => x.SYSTEM_CODE === 'OUT_HUNSAL' || x.CODE === 'OUT_HUNSAL');
            if (hunsalItem) {
                await db.items.update(hunsalItem.ITEM_ID, { SELLING_PRICE: draftHunsalPrice });
            }

            const kuduItem = itemsList.find(x => x.SYSTEM_CODE === 'OUT_KUDU' || x.CODE === 'OUT_KUDU');
            if (kuduItem) {
                await db.items.update(kuduItem.ITEM_ID, { SELLING_PRICE: draftKuduPrice });
            }

            message.success('Catalog selling prices updated in local database!');
            ...
            console.error('Error updating catalog prices in Dexie:', e);
                    ...
                    <div>
                        ...
                        >
                            {outputRiceItems.map(item => (
                                <Option key={item.ITEM_ID} value={item.ITEM_ID} label={item.NAME}>
                                    🌾 {String(item.NAME || 'Rice').toUpperCase()} (Current: Rs. {item.SELLING_PRICE || 0})
                                </Option>
                            ))}
                        </Select>
                    </div>
